"""
Set-DQN curriculum experiment: five picks -> seven picks.

Purpose
-------
Test whether the successful five-pick Set-DQN can be extended to
larger orders using an intermediate seven-pick curriculum stage.

Why seven picks?
----------------
A frozen-policy diagnostic of the five-pick checkpoint showed:

    6 picks  -> 96% completion
    7 picks  -> 86% completion
    8 picks  -> 70% completion
    9 picks  -> 42% completion
    10 picks -> 10% completion

A direct five-to-eight curriculum with epsilon reset to 1.0 did not
improve held-out eight-pick performance: the best checkpoint remained
the transferred episode-0 policy. Seven picks is therefore used as a
more conservative curriculum stage. It retains strong zero-shot
performance while still increasing cardinality beyond the trained
five-pick regime.
...
optimiser state, epsilon schedule, and training counter are deliberately
fresh. Epsilon is reset to 0.30 to preserve more of the already-useful transferred policy while retaining exploration.

Training design
---------------
The warehouse contains 96 fixed pick locations.

Each cycle contains 96 seven-pick orders generated by aligning seven
compatible permutations of all 96 locations. Therefore every physical
location appears exactly seven times per cycle.

With 15 cycles:

    96 x 15 = 1440 training episodes
    7 x 15  = 105 appearances per physical location
...
----------------------
50 deterministic unseen seven-pick combinations are generated starting
at seed 190_000. These are deliberately separate from the earlier
intermediate-cardinality diagnostic set.

The transferred five-pick policy is evaluated on these new seven-pick
probes before any seven-pick training.
...
RUN_NAME = "set_dqn_curriculum_seven_pick_from_five_15_cycles"
...
# Deliberately separate from the earlier seven-pick diagnostic
# seed range (140_000).
PROBE_SEED_START = 190_000
...
LEARNING_RATE = 1e-4
GAMMA = 0.99
# Conservative fine-tuning after the failed 5 -> 8 epsilon=1.0 run.
EPSILON_START = 0.30
EPSILON_MIN = 0.05
...
def create_fresh_agent(env):
    """
    Create a fresh Set-DQN for seven-pick curriculum training.
...
def extract_policy_state_dict(checkpoint):
    ...
    raise RuntimeError(
        "Could not locate policy-network weights in the five-pick "
        "checkpoint.\n"
        f"Checkpoint keys: {list(checkpoint.keys())}"
    )


def load_five_pick_policy_weights(agent, checkpoint_path):
    """
    Transfer only the successful five-pick policy parameters.
    ...
    print("\nLoading five-pick policy weights:")
    ...
    print("Five-pick policy weights transferred successfully.")
    print("Fresh seven-pick optimiser retained.")
...
def create_probe_orders(all_pick_locations, aisle_columns):
    """Generate 50 deterministic unique unseen seven-pick probes."""
    ...
    while len(probe_orders) < PROBE_ORDERS:
        ...
        if attempts > max_attempts:
            raise RuntimeError(
                "Unable to generate enough unique seven-pick "
                "development probes."
            )
...
def build_balanced_cycle_orders(
    ...
    """
    Construct one balanced seven-pick training cycle.

    Seven compatible permutations of all 96 locations are aligned.
    This guarantees exactly seven appearances of every physical
    location during the cycle.

    Constraints:
        - seven distinct locations per order
    ...
    # The seventh permutation must avoid six already-used locations at
    # every aligned position, so allow more attempts than in the
    # lower-cardinality scripts.
    ...
    raise RuntimeError(
        "Unable to construct a valid balanced seven-pick training cycle."
    )
...
def build_balanced_training_schedule(
    ...
    """Build all 15 balanced seven-pick training cycles."""
...
def validate_balanced_training_schedule(
    ...
    if len(training_keys) != len(set(training_keys)):
        raise AssertionError(
            "Training schedule contains duplicate seven-pick combinations."
        )
    ...
    for cycle_number in range(1, TRAIN_CYCLES + 1):
        ...
        for row in cycle_rows:
            ...
            if len(order) != ORDER_SIZE:
                raise AssertionError(
                    "Training order does not contain seven picks."
                )
...
def train_episode(
    ...
    """Train Set-DQN for one seven-pick episode."""
...
def evaluate_dqn_order(
    ...
    """Greedily evaluate one seven-pick development order."""
...
def calculate_probe_optima(grid, depot, probe_orders):
    """Calculate exact optimal distance for all seven-pick probes."""

    optima = []

    print("\nCalculating exact seven-pick probe distances...")
...
def evaluate_probe_set(
    ...
    """Evaluate all unseen seven-pick development orders."""
...
def save_experiment_config(
    ...
    config = {
        "run_name": RUN_NAME,
        "agent": "SetDQNAgent",
        "initialisation": "five_pick_policy_weight_transfer",
        "curriculum": True,
        ...
        "exploration_rationale": (
            "epsilon_reset_to_0.30_after_five_to_eight_epsilon_1.0_"
            "curriculum_failed_to_improve_held_out_performance"
        ),
        "representation": (
        ...
        "probe_policy": "unseen_exact_seven_pick_combinations",
        "probe_reused_from_diagnostic": False,
        "curriculum_selection_basis": (
            "five_pick_frozen_policy_achieved_86_percent_completion_"
            "on_separate_seven_pick_diagnostic_orders"
        ),
        "max_steps": MAX_STEPS,
        "max_order_size": MAX_ORDER_SIZE,
...
def main():
    print(
        "\n"
        "================================================\n"
        "SET-DQN CURRICULUM: FIVE PICKS -> SEVEN PICKS\n"
        "================================================"
    )
    ...
    print(
        f"\nRun name:                 {RUN_NAME}\n"
        f"Agent:                    SetDQN\n"
        f"Initialisation:           five-pick policy transfer\n"
        f"Source order size:        5\n"
    ...
    print("Creating unseen seven-pick development probes...")
    ...
    print(
        f"Created {len(probe_orders)} unique seven-pick "
    ...
    print("\nBuilding balanced seven-pick training schedule...")
    ...
    if maximum_probe_optimum >= MAX_STEPS:
        raise RuntimeError(
            "An seven-pick development probe optimum reaches or "
            "exceeds MAX_STEPS."
        )
    ...
    print(
        "\nEvaluating transferred five-pick policy "
        "on new seven-pick probes BEFORE training..."
    )
    ...
    for schedule_row in training_schedule:
        ...
        if episode % evaluate_every == 0:
            ...
            print("\n------------------------------------------------")
            print(
                f"SET-DQN 5 -> 7 CURRICULUM EVALUATION "
            ...
            if evaluation_is_better(
                ...
                agent.save(best_model_path)
                print("New best seven-pick curriculum model saved.")
    ...
    print(
        "\n"
        "================================================\n"
        "SET-DQN 5 -> 7 CURRICULUM COMPLETE\n"
        "================================================"
    )
    ...
    if best_evaluation_summary is not None:
        print("\nBest seven-pick curriculum checkpoint:")
