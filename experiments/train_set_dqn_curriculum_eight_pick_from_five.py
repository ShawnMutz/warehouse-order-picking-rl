"""
Set-DQN curriculum experiment: five picks -> eight picks.

Purpose
-------
Test whether the successful five-pick Set-DQN can be extended to
larger orders using an intermediate eight-pick curriculum stage.
...
Eight picks is therefore used as the next curriculum target: it is a
substantial increase in cardinality while retaining useful zero-shot
behaviour.
...
fresh. Epsilon is reset to 1.0.

Training design
---------------
The warehouse contains 96 fixed pick locations.

Each cycle contains 96 eight-pick orders generated by aligning eight
compatible permutations of all 96 locations. Therefore every physical
location appears exactly eight times per cycle.

With 20 cycles:

    96 x 20 = 1920 training episodes
    8 x 20  = 160 appearances per physical location
...
----------------------
50 deterministic unseen eight-pick combinations are generated starting
at seed 180_000. These are deliberately separate from the earlier
intermediate-cardinality diagnostic set.

The transferred five-pick policy is evaluated on these new eight-pick
probes before any eight-pick training.
...
RUN_NAME = "set_dqn_curriculum_eight_pick_from_five_20_cycles"
...
# Deliberately separate from the earlier eight-pick diagnostic
# seed range (150_000).
PROBE_SEED_START = 180_000
...
def create_fresh_agent(env):
    """
    Create a fresh Set-DQN for eight-pick curriculum training.
...
def load_five_pick_policy_weights(agent, checkpoint_path):
    ...
    print("Fresh eight-pick optimiser retained.")
...
def create_probe_orders(all_pick_locations, aisle_columns):
    """Generate 50 deterministic unique unseen eight-pick probes."""
    ...
    while len(probe_orders) < PROBE_ORDERS:
        ...
        if attempts > max_attempts:
            raise RuntimeError(
                "Unable to generate enough unique eight-pick "
                "development probes."
            )
...
def build_balanced_cycle_orders(
    ...
    """
    Construct one balanced eight-pick training cycle.

    Eight compatible permutations of all 96 locations are aligned.
    This guarantees exactly eight appearances of every physical
    location during the cycle.

    Constraints:
        - eight distinct locations per order
    ...
    # The eighth permutation must avoid seven already-used locations at
    ...
    raise RuntimeError(
        "Unable to construct a valid balanced eight-pick training cycle."
    )
...
def build_balanced_training_schedule(
    ...
    """Build all 20 balanced eight-pick training cycles."""
...
def validate_balanced_training_schedule(
    ...
    if len(training_keys) != len(set(training_keys)):
        raise AssertionError(
            "Training schedule contains duplicate eight-pick combinations."
        )
    ...
    for cycle_number in range(1, TRAIN_CYCLES + 1):
        ...
        for row in cycle_rows:
            ...
            if len(order) != ORDER_SIZE:
                raise AssertionError(
                    "Training order does not contain eight picks."
                )
...
def train_episode(
    ...
    """Train Set-DQN for one eight-pick episode."""
...
def evaluate_dqn_order(
    ...
    """Greedily evaluate one eight-pick development order."""
...
def calculate_probe_optima(grid, depot, probe_orders):
    """Calculate exact optimal distance for all eight-pick probes."""

    optima = []

    print("\nCalculating exact eight-pick probe distances...")
...
def evaluate_probe_set(
    ...
    """Evaluate all unseen eight-pick development orders."""
...
def save_experiment_config(
    ...
    config = {
        ...
        "probe_policy": "unseen_exact_eight_pick_combinations",
        "probe_reused_from_diagnostic": False,
        "curriculum_selection_basis": (
            "five_pick_frozen_policy_achieved_70_percent_completion_"
            "on_separate_eight_pick_diagnostic_orders"
        ),
        "max_steps": MAX_STEPS,
        "max_order_size": MAX_ORDER_SIZE,
...
def main():
    print(
        "\n"
        "================================================\n"
        "SET-DQN CURRICULUM: FIVE PICKS -> EIGHT PICKS\n"
        "================================================"
    )
    ...
    print("Creating unseen eight-pick development probes...")
    ...
    print(
        f"Created {len(probe_orders)} unique eight-pick "
    ...
    print("\nBuilding balanced eight-pick training schedule...")
    ...
    if maximum_probe_optimum >= MAX_STEPS:
        raise RuntimeError(
            "An eight-pick development probe optimum reaches or "
            "exceeds MAX_STEPS."
        )
    ...
    print(
        ...
        "on new eight-pick probes BEFORE training..."
    )
    ...
    for schedule_row in training_schedule:
        ...
        if episode % evaluate_every == 0:
            ...
            print("\n------------------------------------------------")
            print(
                f"SET-DQN 5 -> 8 CURRICULUM EVALUATION "
            ...
            if evaluation_is_better(
                ...
                agent.save(best_model_path)
                print("New best eight-pick curriculum model saved.")
    ...
    print(
        "\n"
        "================================================\n"
        "SET-DQN 5 -> 8 CURRICULUM COMPLETE\n"
        "================================================"
    )
    ...
    if best_evaluation_summary is not None:
        print("\nBest eight-pick curriculum checkpoint:")
