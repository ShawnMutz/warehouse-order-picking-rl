"""
Balanced two-pick Set-DQN experiment.

Purpose
-------
Test whether the permutation-invariant Set-DQN can generalise to
unseen two-pick combinations.

This intentionally mirrors the earlier fixed-slot coordinate-DQN
two-pick experiment as closely as possible.
...
Each cycle:
    ...
    - none of the fixed development probe pairs may appear
...
Each physical location therefore appears exactly 60 times.

Development evaluation
----------------------
40 fixed two-pick combinations are generated using the same probe
seed range used by the earlier coordinate-DQN experiment.

The exact pair combinations are excluded from training, but their
individual locations remain available in other training pairs.

This therefore evaluates generalisation to unseen combinations of
familiar warehouse locations.
...
The Set-DQN applies the same shared neural encoder to every active
remaining pick and pools the resulting embeddings, making the
representation permutation invariant.

Reward shaping
--------------
Uses the existing potential-based multi-pick shaping implementation.

Exact optimisation is used only during development evaluation.
"""
...
RUN_NAME = "set_dqn_balanced_two_pick_60_cycles"
...
# Same two-pick development design as the earlier
# coordinate-DQN experiment.
PROBE_ORDERS = 40
...
# 96 locations / 2 picks = 48 episodes per balanced cycle.
ROLLING_WINDOW = 48
...
def create_environment(
    ...
    """
    Create WarehouseEnv with fixed project rewards.
    """
...
def order_key(
    ...
    """
    Create an order-independent representation of a pair.
...
def create_probe_orders(
    ...
    """
    Generate deterministic unique two-pick development pairs.

    Uses the same seed range as the earlier coordinate-DQN
    two-pick experiment.
    """
...
def build_balanced_training_schedule(
    ...
    """
    Construct balanced two-pick training cycles.

    During every cycle:

        - all 96 locations appear exactly once
        - locations form 48 two-pick orders
        - no development probe pair appears

    The same deterministic cycle seeding used in the previous
    coordinate-DQN experiment is retained.
    """
    ...
    if len(
        ...
        raise ValueError(
            "Number of warehouse locations must be divisible "
            "by the order size."
        )
    ...
    for cycle_index in range(
        ...
        # ----------------------------------------------------
        # Repeatedly shuffle all locations and pair adjacent
        # elements until no forbidden probe pair occurs.
        # ----------------------------------------------------
        ...
        if valid_pairs is None:
            ...
            raise RuntimeError(
                f"Unable to construct balanced cycle "
                f"{cycle_number} without development pairs."
            )
        ...
        if len(
            ...
            raise AssertionError(
                "Incorrect number of orders in balanced cycle."
            )
...
def validate_balanced_training_schedule(
    ...
    """
    Verify schedule balancing and development-set exclusion.
    """
    ...
    if overlap:
        ...
        raise AssertionError(
            "Development probe pairs were found in training: "
            f"{sorted(overlap)}"
        )

    # --------------------------------------------------------
    # Every location exactly once per cycle
    # --------------------------------------------------------
    ...
    for cycle_number in range(
        ...
        if len(
            ...
            raise AssertionError(
                ...
                f"{len(cycle_rows)} orders; expected "
                f"{orders_per_cycle}."
            )
        ...
        for row in cycle_rows:
            ...
            if len(
                ...
                raise AssertionError(
                    "Training order does not contain two picks."
                )
...
def get_collection_metrics(
    ...
    """
    Calculate pick-collection progress from raw state.
    """
...
def calculate_action_metrics(
    ...
    Valid movement increases travel distance by exactly one.
    """
...
def train_episode(
    ...
    """
    Train Set-DQN for one two-pick episode.
    ...
    while not (
        ...
        if loss is not None:
            ...
            if not np.isfinite(
                ...
                raise RuntimeError(
                    f"Non-finite Set-DQN loss detected: {loss}"
                )
...
def calculate_probe_optima(
    ...
    """
    Calculate exact optimal distance for each development pair.
    """

    optima = []

    print(
        "\nCalculating exact two-pick probe distances..."
    )
...
def evaluate_probe_set(
    ...
    """
    Evaluate all fixed unseen two-pick combinations.
    """
...
def save_experiment_config(
    ...
    """
    Save experimental settings for reproducibility.
    """
    ...
    config = {
        ...
        "comparison":
            "matched_to_previous_coordinate_two_pick_design",
    }
...
def main():
    ...
    print(
        "\n"
        "================================================\n"
        "SET-DQN BALANCED TWO-PICK TRAINING\n"
        "================================================"
    )
    ...
    print(
        "Creating unseen two-pick development probes..."
    )
    ...
    print(
        ...
        "two-pick development combinations."
    )
    ...
    print(
        "\nBuilding balanced two-pick training schedule..."
    )
    ...
    for schedule_row in training_schedule:
        ...
        # ====================================================
        # DEVELOPMENT EVALUATION AFTER EACH CYCLE
        # ====================================================
        ...
        if (
            ...
            print(
                f"SET-DQN TWO-PICK DEVELOPMENT EVALUATION "
            ...
            print(
                ...
                f"{evaluation_summary['mean_items_collected']:.3f}/2"
            )
            ...
            if evaluation_is_better(
                ...
                print(
                    "New best two-pick Set-DQN model saved."
                )
    ...
    print(
        "\n"
        "================================================\n"
        "SET-DQN BALANCED TWO-PICK TRAINING COMPLETE\n"
        "================================================"
    )
    ...
    if best_evaluation_summary is not None:
        ...
        print(
            "\nBest two-pick Set-DQN checkpoint:"
        )
        ...
        print(
            f"  Mean items: "
            f"{best_evaluation_summary['mean_items_collected']:.3f}/2"
        )
