"""
Set-DQN curriculum experiment: fourteen picks -> twenty picks.

Purpose
-------
Test whether the best fourteen-pick Set-DQN can be extended directly to
twenty-pick uniform orders using the established conservative
policy-transfer curriculum.

Why twenty picks?
-----------------
A frozen-policy diagnostic of the fourteen-pick checkpoint, evaluated
with MAX_STEPS=250, showed:

    16 picks -> 96% completion
    17 picks -> 90% completion
    18 picks -> 84% completion
    19 picks -> 80% completion
    20 picks -> 52% completion

The policy therefore already generalises strongly through nineteen
picks, while twenty picks is the first clear remaining cardinality
boundary. A direct fourteen-to-twenty curriculum stage is used rather
than adding unnecessary intermediate stages.
...
configuration used in the later curriculum stages.

Training design
---------------
The warehouse contains 96 fixed pick locations.

Each training cycle contains 96 twenty-pick orders. A shuffled circular
ordering of all 96 locations is created for each cycle and twenty cyclic
shifts are aligned to form the orders. This guarantees:

    - 20 distinct locations in every order
    - 20 appearances of every physical location per cycle
    - 96 orders per cycle

With 8 cycles:

    96 x 8 = 768 training episodes
    20 x 8 = 160 appearances per physical location

Development-probe combinations are excluded and training combinations
are required to be unique across all cycles.

Development evaluation
----------------------
50 deterministic unseen twenty-pick uniform combinations are generated
starting at seed 400_000.

The transferred fourteen-pick policy is evaluated on these probes before
any twenty-pick training. Exact optimal distances are calculated for the
development probes so that completed DQN routes can be compared using
optimality gap.
...
Episode horizon
---------------
MAX_STEPS is set to 250. A paired fifteen-pick sensitivity diagnostic
showed that the earlier 150-step horizon materially censored completion
at larger order sizes. Route inefficiency is still measured directly
through travel distance and optimality gap.
"""
...
RUN_NAME = "set_dqn_curriculum_twenty_pick_from_fourteen_8_cycles"
...
def create_fresh_agent(env):
    """
    Create a fresh Set-DQN for twenty-pick curriculum training.
...
def load_fourteen_pick_policy_weights(agent, checkpoint_path):
    """
    Transfer only the successful fourteen-pick policy parameters.
    ...
    print("\nLoading fourteen-pick policy weights:")
    ...
    print("Fourteen-pick policy weights transferred successfully.")
    print("Fresh twenty-pick optimiser retained.")
...
def create_probe_orders(all_pick_locations, aisle_columns):
    """Generate 50 deterministic unique unseen twenty-pick probes."""
    ...
    while len(probe_orders) < PROBE_ORDERS:
        ...
        if attempts > max_attempts:
            raise RuntimeError(
                "Unable to generate enough unique twenty-pick "
                "development probes."
            )
...
def build_balanced_cycle_orders(
    ...
    """
    Construct one balanced twenty-pick training cycle.

    A shuffled circular ordering of all 96 locations is created. Twenty
    cyclic shifts of that ordering are then aligned. At every aligned
    position the twenty selected locations are distinct, while every
    physical location appears exactly once in each shift.

    Therefore each cycle contains:
        - 96 orders
        - 20 distinct locations per order
        - exactly 20 appearances of every physical location

    The complete candidate cycle is retried if it contains a forbidden
    development-probe combination or a training combination already used
    in an earlier cycle.
    """
    ...
    if ORDER_SIZE > location_count:
        raise ValueError(
            "ORDER_SIZE cannot exceed the number of pick locations."
        )

    max_cycle_attempts = 10_000

    for _ in range(max_cycle_attempts):
        base_permutation = list(locations)
        cycle_rng.shuffle(base_permutation)
        ...
        for position in range(location_count):
            order = [
                base_permutation[
                    (position + slot_index) % location_count
                ]
    ...
    raise RuntimeError(
        "Unable to construct a valid balanced twenty-pick training cycle."
    )
...
def build_balanced_training_schedule(
    ...
    """Build all 8 balanced twenty-pick training cycles."""
    ...
    for cycle_index in range(TRAIN_CYCLES):
        ...
        cycle_rng = random.Random(
            TRAIN_SEED + 200_000 + cycle_index
        )
        ...
        for cycle_position, order in enumerate(
            cycle_orders,
            start=1,
        ):
            episode += 1
...
def validate_balanced_training_schedule(
    ...
    if len(training_keys) != len(set(training_keys)):
        raise AssertionError(
            "Training schedule contains duplicate twenty-pick combinations."
        )
...
def train_episode(
    ...
    """Train Set-DQN for one twenty-pick episode."""
...
def evaluate_dqn_order(
    ...
    """Greedily evaluate one twenty-pick development order."""
...
def calculate_probe_optima(grid, depot, probe_orders):
    """Calculate exact optimal distance for all twenty-pick probes."""

    optima = []

    print("\nCalculating exact twenty-pick probe distances...")
...
def evaluate_probe_set(
    ...
    """Evaluate all unseen twenty-pick development orders."""
...
def save_experiment_config(
    ...
    config = {
        "run_name": RUN_NAME,
        "agent": "SetDQNAgent",
        "initialisation": "fourteen_pick_policy_weight_transfer",
        "curriculum": True,
        "source_model": str(SOURCE_MODEL_PATH),
        "source_order_size": 14,
        ...
        "balanced_schedule_method":
            "shuffled_circular_order_with_twenty_cyclic_shifts",
        ...
        "probe_policy": "unseen_exact_twenty_pick_combinations",
        "probe_reused_from_diagnostic": False,
        "curriculum_selection_basis": (
            "fourteen_pick_frozen_policy_generalised_strongly_through_"
            "nineteen_picks_under_250_steps_but_twenty_pick_completion_"
            "fell_to_52_percent"
        ),
        "max_steps": MAX_STEPS,
        "max_steps_interpretation": (
            "250_steps_adopted_after_paired_fifteen_pick_sensitivity_"
            "analysis_showed_the_150_step_limit_materially_censored_"
...
def main():
    print(
        "\n"
        "================================================\n"
        "SET-DQN CURRICULUM: FOURTEEN PICKS -> TWENTY PICKS\n"
        "================================================"
    )
    ...
    load_fourteen_pick_policy_weights(
        agent,
        SOURCE_MODEL_PATH,
    )
    ...
    print(
        f"\nRun name:                 {RUN_NAME}\n"
        f"Agent:                    SetDQN\n"
        f"Initialisation:           fourteen-pick policy transfer\n"
        f"Source order size:        14\n"
    ...
    print("Creating unseen twenty-pick development probes...")
    ...
    print(
        f"Created {len(probe_orders)} unique twenty-pick "
    ...
    print("\nBuilding balanced twenty-pick training schedule...")
    ...
    if maximum_probe_optimum >= MAX_STEPS:
        raise RuntimeError(
            "A twenty-pick development probe optimum reaches or "
            "exceeds MAX_STEPS."
        )
    ...
    print(
        "\nEvaluating transferred fourteen-pick policy "
        "on new twenty-pick probes BEFORE training..."
    )
    ...
    for schedule_row in training_schedule:
        ...
        if episode % evaluate_every == 0:
            ...
            print("\n------------------------------------------------")
            print(
                f"SET-DQN 14 -> 20 CURRICULUM EVALUATION "
            ...
            if evaluation_is_better(
                ...
                agent.save(best_model_path)
                print("New best twenty-pick curriculum model saved.")
    ...
    print(
        "\n"
        "================================================\n"
        "SET-DQN 14 -> 20 CURRICULUM COMPLETE\n"
        "================================================"
    )
    ...
    if best_evaluation_summary is not None:
        print("\nBest twenty-pick curriculum checkpoint:")
