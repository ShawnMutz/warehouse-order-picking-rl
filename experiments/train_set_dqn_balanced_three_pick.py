"""
Balanced three-pick Set-DQN experiment.

Purpose
-------
Test whether the permutation-invariant Set-DQN can generalise to
unseen three-pick combinations.

This is the decisive representation experiment.

Previous development showed:

    fixed coordinate-slot DQN:
        strong one-pick performance
        strong two-pick performance
        0% unseen three-pick completion

    Set-DQN:
        100% one-pick completion
        100% unseen two-pick completion

This experiment therefore trains the Set-DQN FROM SCRATCH on
three-pick orders.

No curriculum warm-start is used.

Training design
---------------
The warehouse contains 96 fixed pick locations.

Each balanced cycle contains 96 three-pick orders.

Three compatible permutations of all 96 locations are aligned:
...
Therefore, in every cycle:

    96 orders
    3 picks/order
    288 pick appearances

and every physical location appears exactly:
...
and:
    ...
    60 appearances per location.

Development evaluation
----------------------
40 deterministic three-pick probe combinations are generated using
the same seed range used by the earlier fixed-slot three-pick
curriculum experiments:

    PROBE_SEED_START = 80_000

The exact three-location combinations are excluded from training.

Individual locations remain present in other training combinations.

This evaluates combinatorial generalisation to unseen three-pick
orders rather than unseen physical locations.
...
The exact optimiser is used only for development evaluation.
"""
...
RUN_NAME = "set_dqn_balanced_three_pick_20_cycles"
...
# Deliberately matches the earlier fixed-slot three-pick
# curriculum experiments.
PROBE_SEED_START = 80_000
...
# One complete balanced three-pick cycle.
ROLLING_WINDOW = 96
...
def create_environment(
    ...
    """
    Create WarehouseEnv using the fixed project rewards.
    """
...
def create_agent(
    ...
    This experiment deliberately does not load the successful
    one- or two-pick checkpoint.
    """
...
def order_key(
    ...
    """
    Create an order-independent key for a three-pick order.

    Different permutations of the same three locations produce
    the same key.
    """
...
def create_probe_orders(
    ...
    The seed range matches the earlier fixed-slot three-pick
    experiments.
    """
...
def build_balanced_cycle_orders(
    ...
    """
    Construct one balanced three-pick cycle.

    Three compatible permutations of all 96 locations are
    aligned.

    For every aligned position i:
    ...
    This guarantees that every warehouse location appears once
    in each permutation, therefore exactly three times during
    the complete cycle.

    Additional constraints:
        - no duplicate location inside an order
        - no development-probe combination
        - no duplicate training combination within the cycle
        - no duplicate combination used in previous cycles
    """
    ...
    for _ in range(
        ...
        for slot_index in range(
            ...
            for _ in range(
                ...
                # Each aligned position must contain a
        ...
        # ----------------------------------------------------
        # Build the 96 aligned three-pick orders.
        # ----------------------------------------------------
        ...
        for position in range(
            ...
            # Three unique physical locations required.
            if len(
                set(
                    order
                )
            ...
            # No duplicate combination inside this cycle.
            if key in candidate_keys:

                cycle_valid = False
                break

            # No duplicate combination from earlier cycles.
...
def build_balanced_training_schedule(
    ...
    """
    Build the complete 20-cycle three-pick schedule.

    Every cycle contains:
        96 orders
        3 picks/order
        3 appearances/location
...
def validate_balanced_training_schedule(
    ...
    """
    Thoroughly validate the balanced three-pick schedule.
    """
    ...
    # --------------------------------------------------------
    # Training combinations must all be unique.
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # Probe combinations must never appear in training.
    # --------------------------------------------------------
    ...
    if overlap:
        ...
        raise AssertionError(
            "Development-probe combinations found in training: "
            f"{sorted(overlap)}"
        )
    ...
    for cycle_number in range(
        ...
        if len(
            ...
            raise AssertionError(
                ...
                f"{len(cycle_rows)} orders rather than "
                f"{orders_per_cycle}."
            )
...
def calculate_action_metrics(
    ...
    Every valid movement increases travel distance by exactly 1.
    """
...
def train_episode(
    ...
    """
    Train Set-DQN for one three-pick episode.
    """
    ...
    while not (
        ...
        # Replay
        #
        # terminated and truncated remain distinct.
        # ----------------------------------------------------
...
def evaluate_dqn_order(
    ...
    """
    Greedily evaluate one three-pick development order.
    """
...
def calculate_probe_optima(
    ...
    """
    Calculate exact optimal distance for every three-pick probe.
    """
...
def evaluate_probe_set(
    ...
    """
    Evaluate all fixed unseen three-pick development orders.
    """
...
def save_experiment_config(
    ...
    config = {
        ...
        "comparison":
            "three_pick_set_dqn_vs_failed_fixed_slot_three_pick",
    }
...
def main():
    ...
    print(
        "\n"
        "================================================\n"
        "SET-DQN BALANCED THREE-PICK TRAINING\n"
        "================================================"
    )
    ...
    print(
        ...
        "three-pick development combinations."
    )
    ...
    print(
        "\nBuilding balanced three-pick training schedule..."
    )
    ...
    print(
        f"  {exposure_per_location} total appearances/location"
    )

    print(
        f"  {len(training_schedule)} unique training combinations"
    )
    ...
    print(
        "\nEvaluating untrained three-pick Set-DQN..."
    )
    ...
    for schedule_row in training_schedule:
        ...
        if (
            ...
            print(
                f"SET-DQN THREE-PICK DEVELOPMENT EVALUATION "
            ...
            print(
                ...
                f"{evaluation_summary['mean_items_collected']:.3f}/3"
            )
            ...
            if evaluation_is_better(
                ...
                print(
                    "New best three-pick Set-DQN model saved."
                )
    ...
    print(
        "\n"
        "================================================\n"
        "SET-DQN BALANCED THREE-PICK TRAINING COMPLETE\n"
        "================================================"
    )
    ...
    if best_evaluation_summary is not None:
        ...
        print(
            "\nBest three-pick Set-DQN checkpoint:"
        )
        ...
        print(
            f"  Mean items: "
            f"{best_evaluation_summary['mean_items_collected']:.3f}/3"
        )
