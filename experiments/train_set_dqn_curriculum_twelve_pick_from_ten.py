"""
Set-DQN curriculum experiment: ten picks -> twelve picks.

Purpose
-------
Test whether the successful ten-pick Set-DQN can be extended to
twelve-pick orders using the conservative policy-transfer curriculum.

Why twelve picks?
-----------------
A frozen-policy diagnostic of the successful ten-pick checkpoint on
fresh unseen uniform orders showed:

    11 picks -> 86% completion
    12 picks -> 76% completion
    13 picks -> 50% completion
    14 picks -> 44% completion
    15 picks -> 18% completion

Eleven-pick transfer is already strong. Twelve picks retains a strong
transferred policy while leaving meaningful room for curriculum
fine-tuning, whereas the degradation is substantially larger from
thirteen picks onward.
...
strategy used successfully in the later curriculum stages.

Training design
---------------
The warehouse contains 96 fixed pick locations.

Each cycle contains 96 twelve-pick orders generated by aligning twelve
compatible permutations of all 96 locations. Therefore every physical
location appears exactly twelve times per cycle.

With 13 cycles:

    96 x 13 = 1248 training episodes
    12 x 13 = 156 appearances per physical location
...
----------------------
50 deterministic unseen twelve-pick combinations are generated starting
at seed 290_000. These are deliberately separate from the twelve-pick
diagnostic set that began at seed 250_000.

The transferred ten-pick policy is evaluated on these new twelve-pick
probes before any twelve-pick training.
...
Larger-order failures can arise either because the policy fails to
collect/route effectively or because all picks are collected without
returning to the depot within MAX_STEPS. The 150-step limit is held
fixed for consistency with the preceding development experiments.
"""
...
RUN_NAME = "set_dqn_curriculum_twelve_pick_from_ten_13_cycles"
...
# Deliberately separate from the twelve-pick diagnostic
# seed range (250_000).
PROBE_SEED_START = 290_000
...
def create_fresh_agent(env):
    """
    Create a fresh Set-DQN for twelve-pick curriculum training.
...
def extract_policy_state_dict(checkpoint):
    ...
    raise RuntimeError(
        "Could not locate policy-network weights in the ten-pick "
        "checkpoint.\n"
        f"Checkpoint keys: {list(checkpoint.keys())}"
    )


def load_ten_pick_policy_weights(agent, checkpoint_path):
    """
    Transfer only the successful ten-pick policy parameters.
    ...
    print("\nLoading ten-pick policy weights:")
    ...
    print("Ten-pick policy weights transferred successfully.")
    print("Fresh twelve-pick optimiser retained.")
...
def create_probe_orders(all_pick_locations, aisle_columns):
    """Generate 50 deterministic unique unseen twelve-pick probes."""
    ...
    while len(probe_orders) < PROBE_ORDERS:
        ...
        if attempts > max_attempts:
            raise RuntimeError(
                "Unable to generate enough unique twelve-pick "
                "development probes."
            )
...
def build_balanced_cycle_orders(
    ...
    """
    Construct one balanced twelve-pick training cycle.

    Twelve compatible permutations of all 96 locations are aligned.
    This guarantees exactly twelve appearances of every physical
    location during the cycle.

    Constraints:
        - twelve distinct locations per order
    ...
    raise RuntimeError(
        "Unable to construct a valid balanced twelve-pick training cycle."
    )
...
def build_balanced_training_schedule(
    ...
    """Build all 13 balanced twelve-pick training cycles."""
    ...
    for cycle_index in range(TRAIN_CYCLES):
        ...
        cycle_rng = random.Random(
            TRAIN_SEED + 120_000 + cycle_index
        )
...
def validate_balanced_training_schedule(
    ...
    if len(training_keys) != len(set(training_keys)):
        raise AssertionError(
            "Training schedule contains duplicate twelve-pick combinations."
        )
    ...
    for cycle_number in range(1, TRAIN_CYCLES + 1):
        ...
        for row in cycle_rows:
            ...
            if len(order) != ORDER_SIZE:
                raise AssertionError(
                    "Training order does not contain twelve picks."
                )
...
def train_episode(
    ...
    """Train Set-DQN for one twelve-pick episode."""
...
def evaluate_dqn_order(
    ...
    """Greedily evaluate one twelve-pick development order."""
...
def calculate_probe_optima(grid, depot, probe_orders):
    """Calculate exact optimal distance for all twelve-pick probes."""

    optima = []

    print("\nCalculating exact twelve-pick probe distances...")
...
def evaluate_probe_set(
    ...
    """Evaluate all unseen twelve-pick development orders."""
...
def save_experiment_config(
    ...
    config = {
        "run_name": RUN_NAME,
        "agent": "SetDQNAgent",
        "initialisation": "ten_pick_policy_weight_transfer",
        "curriculum": True,
        "source_model": str(SOURCE_MODEL_PATH),
        "source_order_size": 10,
        ...
        "probe_policy": "unseen_exact_twelve_pick_combinations",
        "probe_reused_from_diagnostic": False,
        "curriculum_selection_basis": (
            "ten_pick_frozen_policy_achieved_76_percent_completion_"
            "on_separate_twelve_pick_diagnostic_orders"
        ),
        "max_steps": MAX_STEPS,
        "max_order_size": MAX_ORDER_SIZE,
...
def main():
    print(
        "\n"
        "================================================\n"
        "SET-DQN CURRICULUM: TEN PICKS -> TWELVE PICKS\n"
        "================================================"
    )
    ...
    print(
        f"\nRun name:                 {RUN_NAME}\n"
        f"Agent:                    SetDQN\n"
        f"Initialisation:           ten-pick policy transfer\n"
        f"Source order size:        10\n"
    ...
    print("Creating unseen twelve-pick development probes...")
    ...
    print(
        f"Created {len(probe_orders)} unique twelve-pick "
    ...
    print("\nBuilding balanced twelve-pick training schedule...")
    ...
    if maximum_probe_optimum >= MAX_STEPS:
        raise RuntimeError(
            "A twelve-pick development probe optimum reaches or "
            "exceeds MAX_STEPS."
        )
    ...
    print(
        "\nEvaluating transferred ten-pick policy "
        "on new twelve-pick probes BEFORE training..."
    )
    ...
    for schedule_row in training_schedule:
        ...
        if episode % evaluate_every == 0:
            ...
            print("\n------------------------------------------------")
            print(
                f"SET-DQN 10 -> 12 CURRICULUM EVALUATION "
            ...
            if evaluation_is_better(
                ...
                agent.save(best_model_path)
                print("New best twelve-pick curriculum model saved.")
    ...
    print(
        "\n"
        "================================================\n"
        "SET-DQN 10 -> 12 CURRICULUM COMPLETE\n"
        "================================================"
    )
    ...
    if best_evaluation_summary is not None:
        print("\nBest twelve-pick curriculum checkpoint:")
