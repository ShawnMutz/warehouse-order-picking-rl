"""
Set-DQN curriculum experiment: seven picks -> ten picks.

Purpose
-------
Test whether the successful seven-pick Set-DQN can be extended to
ten-pick orders using the conservative curriculum configuration that
worked for the five-to-seven transition.

Why ten picks?
--------------
A frozen-policy diagnostic of the successful seven-pick checkpoint on
fresh unseen uniform orders showed:

    8 picks  -> 96% completion
    9 picks  -> 88% completion
    10 picks -> 60% completion

Eight- and nine-pick transfer is already strong, so separate training
at those cardinalities would add limited value. Ten picks retains
substantial zero-shot competence while leaving meaningful room for
fine-tuning.
...
optimiser state, epsilon value, and training counter are fresh.

Epsilon is reset to 0.30, retaining the conservative fine-tuning
strategy that successfully improved the five-to-seven curriculum.

Training design
---------------
The warehouse contains 96 fixed pick locations.

Each cycle contains 96 ten-pick orders generated by aligning ten
compatible permutations of all 96 locations. Therefore every physical
location appears exactly ten times per cycle.

With 15 cycles:

    96 x 15 = 1440 training episodes
    10 x 15 = 150 appearances per physical location
...
----------------------
50 deterministic unseen ten-pick combinations are generated starting
at seed 230_000. These are deliberately separate from the fresh
ten-pick diagnostic set that began at seed 220_000.

The transferred seven-pick policy is evaluated on these new ten-pick
probes before any ten-pick training.
...
RUN_NAME = "set_dqn_curriculum_ten_pick_from_seven_15_cycles"
...
# Deliberately separate from the fresh ten-pick diagnostic
# seed range (220_000).
PROBE_SEED_START = 230_000
...
def create_fresh_agent(env):
    """
    Create a fresh Set-DQN for ten-pick curriculum training.
...
def extract_policy_state_dict(checkpoint):
    ...
    raise RuntimeError(
        "Could not locate policy-network weights in the seven-pick "
        "checkpoint.\n"
        f"Checkpoint keys: {list(checkpoint.keys())}"
    )


def load_seven_pick_policy_weights(agent, checkpoint_path):
    """
    Transfer only the successful seven-pick policy parameters.
    ...
    print("\nLoading seven-pick policy weights:")
    ...
    print("Seven-pick policy weights transferred successfully.")
    print("Fresh ten-pick optimiser retained.")
...
def create_probe_orders(all_pick_locations, aisle_columns):
    """Generate 50 deterministic unique unseen ten-pick probes."""
    ...
    while len(probe_orders) < PROBE_ORDERS:
        ...
        if attempts > max_attempts:
            raise RuntimeError(
                "Unable to generate enough unique ten-pick "
                "development probes."
            )
...
def build_balanced_cycle_orders(
    ...
    """
    Construct one balanced ten-pick training cycle.

    Ten compatible permutations of all 96 locations are aligned.
    This guarantees exactly ten appearances of every physical
    location during the cycle.

    Constraints:
        - ten distinct locations per order
    ...
    raise RuntimeError(
        "Unable to construct a valid balanced ten-pick training cycle."
    )
...
def build_balanced_training_schedule(
    ...
    """Build all 15 balanced ten-pick training cycles."""
    ...
    for cycle_index in range(TRAIN_CYCLES):
        ...
        cycle_rng = random.Random(
            TRAIN_SEED + 100_000 + cycle_index
        )
...
def validate_balanced_training_schedule(
    ...
    if len(training_keys) != len(set(training_keys)):
        raise AssertionError(
            "Training schedule contains duplicate ten-pick combinations."
        )
    ...
    for cycle_number in range(1, TRAIN_CYCLES + 1):
        ...
        for row in cycle_rows:
            ...
            if len(order) != ORDER_SIZE:
                raise AssertionError(
                    "Training order does not contain ten picks."
                )
...
def train_episode(
    ...
    """Train Set-DQN for one ten-pick episode."""
...
def evaluate_dqn_order(
    ...
    """Greedily evaluate one ten-pick development order."""
...
def calculate_probe_optima(grid, depot, probe_orders):
    """Calculate exact optimal distance for all ten-pick probes."""

    optima = []

    print("\nCalculating exact ten-pick probe distances...")
...
def evaluate_probe_set(
    ...
    """Evaluate all unseen ten-pick development orders."""
...
def save_experiment_config(
    ...
    config = {
        "run_name": RUN_NAME,
        "agent": "SetDQNAgent",
        "initialisation": "seven_pick_policy_weight_transfer",
        "curriculum": True,
        ...
        "probe_policy": "unseen_exact_ten_pick_combinations",
        "probe_reused_from_diagnostic": False,
        "curriculum_selection_basis": (
            "seven_pick_frozen_policy_achieved_60_percent_completion_"
            "on_separate_ten_pick_diagnostic_orders"
        ),
        "max_steps": MAX_STEPS,
        "max_order_size": MAX_ORDER_SIZE,
...
def main():
    print(
        "\n"
        "================================================\n"
        "SET-DQN CURRICULUM: SEVEN PICKS -> TEN PICKS\n"
        "================================================"
    )
    ...
    load_seven_pick_policy_weights(
        agent,
        SOURCE_MODEL_PATH,
    )
    ...
    print(
        f"\nRun name:                 {RUN_NAME}\n"
        f"Agent:                    SetDQN\n"
        f"Initialisation:           seven-pick policy transfer\n"
        f"Source order size:        7\n"
    ...
    print("Creating unseen ten-pick development probes...")
    ...
    print(
        f"Created {len(probe_orders)} unique ten-pick "
    ...
    print("\nBuilding balanced ten-pick training schedule...")
    ...
    if maximum_probe_optimum >= MAX_STEPS:
        raise RuntimeError(
            "A ten-pick development probe optimum reaches or "
            "exceeds MAX_STEPS."
        )
    ...
    print(
        "\nEvaluating transferred seven-pick policy "
        "on new ten-pick probes BEFORE training..."
    )
    ...
    for schedule_row in training_schedule:
        ...
        if episode % evaluate_every == 0:
            ...
            print("\n------------------------------------------------")
            print(
                f"SET-DQN 7 -> 10 CURRICULUM EVALUATION "
            ...
            if evaluation_is_better(
                ...
                agent.save(best_model_path)
                print("New best ten-pick curriculum model saved.")
    ...
    print(
        "\n"
        "================================================\n"
        "SET-DQN 7 -> 10 CURRICULUM COMPLETE\n"
        "================================================"
    )
    ...
    if best_evaluation_summary is not None:
        print("\nBest ten-pick curriculum checkpoint:")
